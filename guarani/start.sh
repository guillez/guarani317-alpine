...
mv root/instalar.sh /data/cfg
mv root/restaurar.sh /data/cfg
mv root/jasper.sh /data/cfg
mv root/variables /data/cfg
...
# Configurar aqui aspectos de url Guarani
if [ ! -z "$APACHE_SERVER_NAME" ]
	then
		sed -i "s/#ServerName www.example.com:80/ServerName $APACHE_SERVER_NAME/" /etc/apache2/httpd.conf
		echo "Cambiando nombre de servidor a.. '$APACHE_SERVER_NAME'..."
	else
		echo "INFORME: Cambie 'ServerName'  >> 'APACHE_SERVER_NAME=your.server.name' "
fi
...
echo "Reiniciando procesos de apache..."
